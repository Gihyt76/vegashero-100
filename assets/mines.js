...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-d2ca535b";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-d2ca535b">'+'<span class="tile-face-d2ca535b tile-front-d2ca535b"></span>'+'<span class="tile-face-d2ca535b tile-back-d2ca535b"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-d2ca535b")){return;}
var back=tile.querySelector(".tile-back-d2ca535b");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-d2ca535b");if(isMine){tile.classList.add("boom-d2ca535b");}
if(dim){tile.classList.add("dimmed-d2ca535b");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-d2ca535b")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-d2ca535b");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-d2ca535b")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-d2ca535b");void panelEl.offsetWidth;panelEl.classList.add("jolt-d2ca535b");}
endRound();}
